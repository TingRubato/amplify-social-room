...
const Header: React.FC = () => {
  useEffect(() => {
    ...
    if (ctx) ctx.scale(dpr, dpr); // to scale every drawing operations
    
    const canvasMw = document.getElementById("milkyWayCanvas") as HTMLCanvasElement;
    canvasMw.width = window.innerWidth * dpr;
    canvasMw.height = window.innerHeight * dpr;
    const ctxMw = canvasMw.getContext('2d');
    if (ctxMw) ctxMw.scale(dpr, dpr); // to scale every drawing operations
    ...
    const mwAngle = 0.4;            // to incline the milky way (0 is horizontal, tend to infinite to get vertical)
